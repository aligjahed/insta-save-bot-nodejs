...
app.help(async (ctx) => {
  ...
  await ctx.replyWithPhoto(
    "https://www.dropbox.com/s/pb3zduqnehxmmyt/2.jpg?raw=1"
  );
  await ctx.reply("سپس بر روی سه نقطه بالای پست کلیک کنید");
  await ctx.replyWithPhoto(
    "https://www.dropbox.com/s/kyd2e4pqz10kdy7/3.jpg?raw=1"
  );
  await ctx.reply("با کلیک بر روی گزینه Link لینک پست را کپی کنید. ");
  await ctx.replyWithPhoto(
    "https://www.dropbox.com/s/haoo6tv2aqfgbjx/4.jpg?raw=1"
  );
  await ctx.reply(
    "به تلگرام برگشته و در صفحه ربات دستور /getPost را وارد نمایید."
  );
  await ctx.replyWithPhoto(
    "https://www.dropbox.com/s/uobren8v7iglj9b/5.jpg?raw=1"
  );
  await ctx.reply(
  ...
  await ctx.replyWithPhoto(
    "https://www.dropbox.com/s/ri7chfi75rqffjr/6.jpg?raw=1"
  );
  await ctx.reply(
